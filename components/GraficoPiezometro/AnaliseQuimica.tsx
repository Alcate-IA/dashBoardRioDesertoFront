...
export default function AnaliseQuimica({ data, carregando }: AnaliseQuimicaProps) {
    ...
    const extrairValorNumerico = (valor: string): number | null => {
        if (!valor || valor === 'ND' || valor === 'Ausente' || valor === '< 0,1' || valor === 'ND') return 0;
    ...
    const verificarLimite = (parametroChave: string, valor: string): { 
        atende: boolean; 
        mensagem: string;
        ...
        if (config.tipo === 'ausente') {
            ...
            const atende = valorLower === 'ausente' || valorLower === '0' || valor === '< 0,1';
            return { 
                atende, 
        ...
        if (config.tipo === 'pH') {
            atende = valorNumerico >= (config.min || 0) && valorNumerico <= (config.max || 0);
            mensagem = atende 
    ...
    const obterCorCard = (status: 'ok' | 'alerta' | 'ausente') => {
        switch (status) {
            case 'ok': return 'bg-gray-800 border-green-500';
            case 'alerta': return 'bg-red-900/30 border-red-500';
            case 'ausente': return 'bg-yellow-900/30 border-yellow-500';
    ...
    return (
        <div className="p-3">
            ...
            <div className="mb-4">
                <h6 className="font-bold mb-2 text-white">Parâmetros da Portaria GM/MS nº 888/2021</h6>
                <div className="grid">
                    {Object.entries(parametrosPortaria).map(([chave, parametro]) => {
                        const valorNosDados = analises?.[parametro.chaveBackend] || '';
                        const { atende, mensagem, status } = verificarLimite(chave, valorNosDados);
                        
                        return (
                            <div key={chave} className="col-12 md:col-6 lg:col-3 mb-2">
                                <div className={`border-1 p-3 border-round ${obterCorCard(status)}`}>
                                    <div className="flex justify-content-between align-items-start mb-2">
                                        ...
                                        {status === 'ok' && (
                                            <Badge value="OK" severity="success" className="text-xs" />
                                        )}
                                        {status === 'alerta' && (
                                            <Badge value="Alerta" severity="danger" className="text-xs" />
                                        )}
                                        {status === 'ausente' && (
                                            <Badge value="N/D" severity="warning" className="text-xs" />
                                        )}
                                    </div>
                                    
                                    <div className="mt-2">
                                        {valorNosDados ? (
                                            <div className={`font-bold ${status === 'alerta' ? 'text-red-300' : 'text-white'}`}>
                                                {valorNosDados}
                                            </div>
                                        ) : (
